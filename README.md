...
- На полученных признаках строятся модели кластеризации KMeans. Оптимальное число параметров определяется "методом локтя" (оптимальным оказалось 8-12 кластеров, для анализа было взято 10).
